...
void AVL::begin_of_writing(std::string path) {
	std::ofstream fout(path, std::ios::binary);
	if (!root) {
		char symbol = '#';
		fout.write(reinterpret_cast<char*>(&symbol), sizeof(char));
	}
	else {
		writing(root, fout);
	}
...
void AVL::writing(Node* node, std::ofstream& fout) {
	if (!node) {
		char end = '#';
		fout.write(reinterpret_cast<char*>(&end), sizeof(char));
		return;
	}
	else {
		char begin = '|';
		std::string key = node->get_key();
		short balance = node->get_balance(), length = key.length();
		unsigned long long value = node->get_value();
		fout.write(reinterpret_cast<char*>(&begin), sizeof(char));
		fout.write(reinterpret_cast<char*>(&balance), sizeof(short));
		fout.write(reinterpret_cast<char*>(&value), sizeof(unsigned long long));
		fout.write(reinterpret_cast<char*>(&length), sizeof(short));
		fout.write(key.data(), length);
		writing(node->get_left(), fout);
		writing(node->get_right(), fout);
	}
}

void AVL::begin_of_reading(std::string path) {
	std::ifstream fin(path, std::ios::binary);
	char first;
	fin.read(reinterpret_cast<char*>(&first), sizeof(char));
	if (first == '#') {
		root = nullptr;
	}
	else if (first == '|') {
		...
		fin.read(reinterpret_cast<char*>(&balance), sizeof(short));
		fin.read(reinterpret_cast<char*>(&value), sizeof(unsigned long long));
		fin.read(reinterpret_cast<char*>(&length), sizeof(short));
		char* buffer = new char[length + 1];
		fin.read(buffer, length);
		buffer[length] = '\0';
		root = new Node(std::string(buffer), value, nullptr);
		root->set_balance(balance);
		delete[] buffer;
		reading(root, fin, true);
		reading(root, fin, false);
	}
	fin.close();
}

void AVL::reading(Node* parent, std::ifstream& fin, bool left) {
	char first;
	fin.read(reinterpret_cast<char*>(&first), sizeof(char));
	if (first == '#') {
		return;
		...
		fin.read(buffer, length);
		buffer[length] = '\0';
		Node* current = new Node(std::string(buffer), value, parent);
		current->set_balance(balance);
		if (left) {
		...
		delete[] buffer;
		reading(current, fin, true);
		reading(current, fin, false);
	}
}
